...
_impl_install() {
  ...
  loading "Installing OpenSpec" _impl_install_impl
}
...
_impl_update() {
  _impl_require_npm
  loading "Updating OpenSpec" _impl_update_impl || { log_error "Failed to update OpenSpec"; return 1; }
...
_impl_vlocal() {
  _spin_capture "Detecting Openspec version" bash -c 'command -v openspec >/dev/null 2>&1 && openspec --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Openspec updates" bash -c "npm view @fission-ai/openspec version 2>/dev/null | head -1"
}
