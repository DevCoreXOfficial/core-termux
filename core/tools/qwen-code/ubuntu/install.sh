...
_impl_install() {
  ...
  loading "Installing Qwen Code" _impl_install_impl
}

_impl_install_impl() {
_impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g @qwen-code/qwen-code &>>"$LOG_FILE"
}
...
_impl_update() {
  _impl_require_npm
  loading "Updating Qwen Code" _impl_update_impl || { log_error "Failed to update Qwen Code"; return 1; }
...
_impl_vremote() {
  _spin_capture "Checking Qwen Code updates" bash -c "npm view @qwen-code/qwen-code version 2>/dev/null | head -1"
}
