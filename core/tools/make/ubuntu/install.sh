...
_impl_install() {
  ...
  loading "Installing Make" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Make version" bash -c "dpkg -s build-essential 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Make updates" bash -c "apt-cache policy build-essential 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Make" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
