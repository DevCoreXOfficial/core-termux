...
_impl_install() {
  ...
  loading "Installing MiniMax" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v minimax 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update() {
  _impl_require_npm
  loading "Updating MiniMax Agent" _impl_update_impl || { log_error "Failed to update MiniMax Agent"; return 1; }
  log_success "MiniMax Agent updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g mmx-cli@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Minimax version" bash -c 'command -v mmx >/dev/null 2>&1 && mmx --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Minimax updates" bash -c "npm view mmx-cli version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "MiniMax CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
