...
_impl_update_impl() {
  _npm_g install -g mmx-cli@latest &>>"$LOG_FILE"
}

_impl_vlocal() {
  _spin_capture "Detecting Minimax version" bash -c "command -v mmx-cli >/dev/null 2>&1 && mmx-cli --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+[^ ]*' | head -1"
}
