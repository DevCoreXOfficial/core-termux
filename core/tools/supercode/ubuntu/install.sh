...
_impl_update_impl() {
  _npm_g install -g supercode-cli@latest &>>"$LOG_FILE"
}

_impl_vlocal() {
  _spin_capture "Detecting Supercode version" bash -c "command -v supercode-cli >/dev/null 2>&1 && supercode-cli --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+[^ ]*' | head -1"
}
