...
_impl_install() {
  ...
  loading "Installing SuperCode" _impl_install_impl
}

_impl_install_impl() {
_impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g supercode-cli &>>"$LOG_FILE"
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v supercode 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
    rm -f "$bin_path" || $CORE_SUDO rm -f "$bin_path" || true
  fi
  _npm_g uninstall -g supercode-cli &>>"$LOG_FILE" || true
}

_impl_update() {
  _impl_require_npm
  loading "Updating Supercode CLI" _impl_update_impl || { log_error "Failed to update Supercode CLI"; return 1; }
  log_success "Supercode CLI updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g supercode-cli@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Supercode version" bash -c 'command -v supercode >/dev/null 2>&1 && supercode --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "SuperCode CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
