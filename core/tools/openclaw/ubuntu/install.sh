...
_impl_install() {
  ...
  loading "Installing OpenClaw" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v openclaw 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update_impl() {
  _npm_g install -g openclaw@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Openclaw version" bash -c 'command -v openclaw >/dev/null 2>&1 && openclaw --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Openclaw updates" bash -c "npm view openclaw version 2>/dev/null | head -1"
}
