...
_compile_kilocode_helper_impl() {
  local HELPER_SRC="$CORE_PATH/tools/ai/kilocode/helper/kilocode_helper.c"
...
_kilocode_create_proot_wrapper() {
  local wrapper_src="$CORE_PATH/tools/ai/kilocode/bin/kilocode.proot"
...
_kilocode_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_PATH/tools/ai/kilocode/bin/kilocode"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_kilocode_cli; fi
if [[ "${1:-}" == "uninstall" ]]; then uninstall_kilocode_cli; fi
