...
_impl_install() {
  ...
  loading "Installing KiloCode" _impl_install_impl
}

_impl_install_impl() {
_impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g @kilocode/cli &>>"$LOG_FILE"
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v kilocode 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
    rm -f "$bin_path" || $CORE_SUDO rm -f "$bin_path" || true
  fi
  _npm_g uninstall -g @kilocode/cli &>>"$LOG_FILE" || true
}

_impl_update() {
  _impl_require_npm
  loading "Updating KiloCode CLI" _impl_update_impl || { log_error "Failed to update KiloCode CLI"; return 1; }
  log_success "KiloCode CLI updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g @kilocode/cli@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Kilocode version" bash -c 'command -v kilocode >/dev/null 2>&1 && kilocode --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Kilocode updates" bash -c "npm view @kilocode/cli version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "KiloCode CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
