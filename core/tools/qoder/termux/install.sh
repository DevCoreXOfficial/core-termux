...
_compile_qoder_helper_impl() {
  local HELPER_SRC="$CORE_TOOL_DIR/helper/qoder_helper.c"
...
_qoder_create_proot_wrapper() {
  local wrapper_src="$CORE_TOOL_DIR/bin/qodercli.proot"
...
_qoder_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_TOOL_DIR/bin/qodercli"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_qoder; fi
