...
_impl_vlocal() {
  pipx list 2>/dev/null | grep -A1 '^   package mistral-vibe' | grep -oE '[0-9]+\.[0-9]+[^ ]*' | head -1
}

_impl_vremote() {
  curl -fsSL https://pypi.org/pypi/mistral-vibe/json 2>/dev/null | grep -o '"version":"[^"]*"' | head -1 | cut -d'"' -f4
}
