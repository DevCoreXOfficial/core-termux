...
_impl_install() {
  ...
  loading "Installing Mistral Vibe" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  command -v "vibe" >/dev/null 2>&1 && rm -f "$(command -v vibe)"
}

_impl_update() {
  loading "Updating Mistral Vibe" _impl_update_impl || { log_error "Failed to update Mistral Vibe"; return 1; }
...
_impl_vlocal() {
  _get_installed_version "vibe" "--version" "Mistral Vibe"
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Mistral Vibe" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
