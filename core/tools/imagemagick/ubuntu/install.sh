...
_impl_install() {
  ...
  loading "Installing ImageMagick" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Imagemagick version" bash -c "dpkg -s imagemagick 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Imagemagick updates" bash -c "apt-cache policy imagemagick 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
