...
_impl_install() {
  ...
  loading "Installing Vercel" _impl_install_impl
}
...
_impl_uninstall() {
  if ! command -v vercel &>/dev/null; then
    ...
    return 2
  local bin_path
  bin_path="$(command -v vercel 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update() {
  _impl_require_npm
  loading "Updating Vercel CLI" _impl_update_impl || { log_error "Failed to update Vercel CLI"; return 1; }
  log_success "Vercel CLI updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g vercel@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Vercel version" bash -c 'command -v vercel >/dev/null 2>&1 && vercel --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Vercel updates" bash -c "npm view vercel version 2>/dev/null | head -1"
}
