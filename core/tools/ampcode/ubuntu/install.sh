...
_impl_install() {
  _impl_require_npm
  npm install -g @ampcode/cli &>>"$LOG_FILE"
}

_impl_uninstall() {
  npm uninstall -g @ampcode/cli &>>"$LOG_FILE" || true
}

_impl_update() {
  _impl_require_npm
  npm install -g @ampcode/cli@latest &>>"$LOG_FILE"
}

_impl_vlocal() {
  npm ls -g @ampcode/cli --depth=0 2>/dev/null | grep '@' | sed 's/.*@//' | head -1
}

_impl_vremote() {
  npm view @ampcode/cli version 2>/dev/null | head -1
}
