...
_impl_install() {
  ...
  loading "Installing SQLite" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Sqlite version" bash -c "dpkg -s sqlite3 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Sqlite updates" bash -c "apt-cache policy sqlite3 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
