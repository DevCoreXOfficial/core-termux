#!/usr/bin/env bash
# Platform: Ubuntu Linux / Ubuntu (WSL). Official installation method.
# Verbs: install | uninstall | update | reinstall | version-local | version-remote
CORE_TOOL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -n "$CORE_PATH" ]] || CORE_PATH="$HOME/.core/core"
source "$CORE_PATH/utils/bootstrap.sh"
import "@/utils/env"
import "@/utils/log"
import "@/lib/platform"
import "@/lib/engine"
core_detect_platform

LOG_FILE="${LOG_FILE:-$CORE_CACHE/install.log}"

_impl_install() {
  separator
  box_large "Installing MimoCode"
  separator
  echo

  loading "Installing MimoCode" _impl_install_impl
}

_impl_install_impl() {
mkdir -p "$HOME/.local/bin"
  curl -fsSL https://mimo.xiaomi.com/install | bash &>>"$LOG_FILE"
  # Expose binaries from well-known script locations.
  for d in "$HOME/.local/bin" "$HOME/bin"; do [[ -d "$d" ]] && case ":$PATH:" in *":$d:"*) ;; *) export PATH="$d:$PATH";; esac; done
}

_impl_uninstall() {
  separator
  box_large "Uninstalling MimoCode"
  separator
  echo

  log_info "Removing binaries..."
  command -v "mimo" >/dev/null 2>&1 && rm -f "$(command -v mimo)"
}

_impl_update() {
  loading "Updating MiMo Code" _impl_update_impl || { log_error "Failed to update MiMo Code"; return 1; }
  log_success "MiMo Code updated to the latest version"
}

_impl_update_impl() {
  curl -fsSL https://mimo.xiaomi.com/install | bash &>>"$LOG_FILE"
}

_impl_vlocal() {
  _spin_capture "Detecting Mimocode version" bash -c 'command -v mimo >/dev/null 2>&1 && mimo --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Mimocode updates" bash -c 'curl -fsSL https://api.github.com/repos/XiaomiMiMo/MiMo-Code/releases/latest | grep "\"tag_name\"" | cut -d"\"" -f4 | sed "s/^v//"' 
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "MiMoCode" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
  version-local)  _impl_vlocal ;;
  version-remote) _impl_vremote ;;
  reinstall)  _impl_install ;;
  *)
    exit 0
    ;;
esac
