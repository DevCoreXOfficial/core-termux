...
_impl_install() {
  ...
  loading "Installing OpenSSH" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Openssh version" bash -c "dpkg -s openssh-server 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Openssh updates" bash -c "apt-cache policy openssh-server 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
