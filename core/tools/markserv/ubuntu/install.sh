...
_impl_install() {
  ...
  loading "Installing Markserv" _impl_install_impl
}

_impl_install_impl() {
  _impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g markserv &>>"$LOG_FILE"
}

_impl_uninstall() {
  _npm_g uninstall -g markserv &>>"$LOG_FILE" || true
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Markserv" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
