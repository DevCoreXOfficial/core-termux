...
AGY_DATA_DIR="$HOME/.local/share/core-termux-data/antigravity"
MANIFEST_URL="https://antigravity-auto-updater-974169037036.us-central1.run.app/manifests/linux_arm64.json"
...
_antigravity_compile_helper_impl() {
  local HELPER_SRC="$CORE_PATH/tools/ai/antigravity/helper/agy_helper.c"
...
_antigravity_create_proot_wrapper() {
  local wrapper_src="$CORE_PATH/tools/ai/antigravity/bin/agy.proot"
...
_antigravity_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_PATH/tools/ai/antigravity/bin/agy"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
uninstall_antigravity_cli() {
  ...
  if [ -f "$ubuntu_bashrc" ]; then
    sed -i '/# antigravity/d; /export PATH=\/root\/.local\/bin/d' "$ubuntu_bashrc"
  fi
...
if [[ "${1:-}" == "install" ]]; then install_antigravity_cli; fi
