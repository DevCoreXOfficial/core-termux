...
_impl_update() {
  _impl_require_npm
  loading "Updating Localtunnel" _impl_update_impl || { log_error "Failed to update Localtunnel"; return 1; }
...
_impl_vlocal() {
  _spin_capture "Detecting Localtunnel version" bash -c "npm ls -g localtunnel --depth=0 2>/dev/null | grep '@' | sed 's/.*@//' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Localtunnel updates" bash -c "npm view localtunnel version 2>/dev/null | head -1"
}
