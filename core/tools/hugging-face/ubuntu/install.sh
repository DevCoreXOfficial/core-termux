...
_impl_install() {
  ...
  loading "Installing Hugging Face" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  command -v "hf" >/dev/null 2>&1 && rm -f "$(command -v hf)"
}

_impl_update() {
  loading "Updating Hugging Face" _impl_update_impl || { log_error "Failed to update Hugging Face"; return 1; }
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Hugging Face CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
