...
_compile_claude_helper_impl() {
  local HELPER_SRC="$CORE_TOOL_DIR/helper/claude_helper.c"
...
_claude_create_proot_wrapper() {
  local wrapper_src="$CORE_TOOL_DIR/bin/claude.proot"
...
_claude_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_TOOL_DIR/bin/claude"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_claude_code; fi
