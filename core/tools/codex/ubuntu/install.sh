...
_impl_install() {
  ...
  loading "Installing Codex" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v codex 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update_impl() {
  _npm_g install -g @openai/codex@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Codex version" bash -c 'command -v codex >/dev/null 2>&1 && codex --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Codex updates" bash -c "npm view @openai/codex version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Codex CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
