...
_impl_update() {
  _impl_require_npm
  loading "Updating npm-check-updates" _impl_update_impl || { log_error "Failed to update npm-check-updates"; return 1; }
...
_impl_vlocal() {
  _spin_capture "Detecting Ncu version" bash -c 'command -v npm-check-updates >/dev/null 2>&1 && npm-check-updates --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Ncu updates" bash -c "npm view npm-check-updates version 2>/dev/null | head -1"
}
