...
_impl_install() {
  ...
  loading "Installing Context7" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v ctx7 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update() {
  _impl_require_npm
  loading "Updating Context7" _impl_update_impl || { log_error "Failed to update Context7"; return 1; }
  log_success "Context7 updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g ctx7@latest
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Context7" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
