...
_impl_install() {
  ...
  loading "Installing Cline" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v cline 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update() {
  _impl_require_npm
  loading "Updating Cline CLI" _impl_update_impl || { log_error "Failed to update Cline CLI"; return 1; }
  log_success "Cline CLI updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g cline@latest
}
...
_impl_vremote() {
  _spin_capture "Checking Cline updates" bash -c "npm view cline version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Cline CLI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
