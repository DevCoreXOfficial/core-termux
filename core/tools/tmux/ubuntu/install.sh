...
_impl_install() {
  ...
  loading "Installing tmux" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Tmux version" bash -c "dpkg -s tmux 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Tmux updates" bash -c "apt-cache policy tmux 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
