...
_impl_install() {
  ...
  chmod +x "$HOME/.local/bin/gentle-ai" 2>/dev/null || true
}
...
_impl_vlocal() {
  "$HOME/.local/bin/gentle-ai" --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1
}

_impl_vremote() {
  curl -fsSL https://api.github.com/repos/Gentleman-Programming/gentle-ai/releases/latest | grep '"tag_name"' | cut -d'"' -f4 | tr -d v
}
