...
_impl_install() {
  ...
  loading "Installing Gentle AI" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  command -v "gentle-ai" >/dev/null 2>&1 && rm -f "$(command -v gentle-ai)"
}
...
_impl_vlocal() {
  _get_installed_version "gentle-ai" "--version" "Gentle AI"
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Gentle AI" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
