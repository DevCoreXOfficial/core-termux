...
_build_and_apply_patches_impl() {
  local patcher_src="$CORE_TOOL_DIR/termux-patches.go"
...
if [[ "${1:-}" == "install" ]]; then install_gentle_ai; fi
if [[ "${1:-}" == "uninstall" ]]; then uninstall_gentle_ai; fi
if [[ "${1:-}" == "update" ]]; then update_gentle_ai; fi
if [[ "${1:-}" == "reinstall" ]]; then reinstall_gentle_ai; fi
if [[ "${1:-}" == "version-local" ]]; then _get_installed_version gentle-ai; fi
if [[ "${1:-}" == "version-remote" ]]; then _get_remote_github_version Gentleman-Programming/gentle-ai; fi
