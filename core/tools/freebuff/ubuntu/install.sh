...
_impl_install() {
  ...
  loading "Installing FreeBuff" _impl_install_impl
}

_impl_install_impl() {
_impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g freebuff &>>"$LOG_FILE"
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v freebuff 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
    rm -f "$bin_path" || $CORE_SUDO rm -f "$bin_path" || true
  fi
  _npm_g uninstall -g freebuff &>>"$LOG_FILE" || true
}

_impl_update() {
  _impl_require_npm
  loading "Updating freebuff" _impl_update_impl || { log_error "Failed to update freebuff"; return 1; }
  log_success "freebuff updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g freebuff@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Freebuff version" bash -c 'command -v freebuff >/dev/null 2>&1 && freebuff --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Freebuff updates" bash -c "npm view freebuff version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Freebuff" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
