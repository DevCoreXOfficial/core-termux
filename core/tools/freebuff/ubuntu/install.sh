...
_impl_install() {
  ...
  _gh_download "CodebuffAI/codebuff-community" "freebuff-linux-" "/tmp/freebuff.tar.gz" && tar -xzf "/tmp/freebuff.tar.gz" -C /tmp &>>"$LOG_FILE"
  BIN=$(find /tmp -maxdepth 2 -type f -name freebuff | head -1)
  [ -z "$BIN" ] && { log_error "binary not found after extraction"; return 1; }
  mv "$BIN" "$HOME/.local/bin/freebuff" && chmod +x "$HOME/.local/bin/freebuff"
