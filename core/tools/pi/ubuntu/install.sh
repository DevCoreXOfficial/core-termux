...
_impl_install() {
  ...
  loading "Installing PI" _impl_install_impl
}
...
_impl_update() {
  loading "Updating Pi Coding Agent" _impl_update_impl || { log_error "Failed to update Pi Coding Agent"; return 1; }
...
_impl_vlocal() {
  _get_installed_version "pi" "--version" "PI"
}
