...
_impl_install() {
  ...
  loading "Installing wget" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Wget version" bash -c "dpkg -s wget 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Wget updates" bash -c "apt-cache policy wget 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
