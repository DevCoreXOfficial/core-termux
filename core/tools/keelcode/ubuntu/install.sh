...
_impl_update() {
  _impl_require_npm
  loading "Updating KeelCode CLI" _impl_update_impl || { log_error "Failed to update KeelCode CLI"; return 1; }
...
_impl_vlocal() {
  _spin_capture "Detecting Keelcode version" bash -c "npm ls -g @keelcode-ai/keelcode --depth=0 2>/dev/null | grep '@' | sed 's/.*@//' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Keelcode updates" bash -c "npm view @keelcode-ai/keelcode version 2>/dev/null | head -1"
}
