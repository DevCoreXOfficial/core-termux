...
_impl_install() {
  ...
  loading "Installing ngrok" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Ngrok version" bash -c 'command -v ngrok >/dev/null 2>&1 && ngrok --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "ngrok" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
