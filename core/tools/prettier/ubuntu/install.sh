...
_impl_install() {
  ...
  loading "Installing Prettier" _impl_install_impl
}
...
_impl_uninstall() {
  if ! command -v prettier &>/dev/null; then
    ...
    return 2
  local bin_path
  bin_path="$(command -v prettier 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update() {
  _impl_require_npm
  loading "Updating Prettier" _impl_update_impl || { log_error "Failed to update Prettier"; return 1; }
  log_success "Prettier updated to the latest version"
}

_impl_update_impl() {
  _npm_g install -g prettier@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Prettier version" bash -c 'command -v prettier >/dev/null 2>&1 && prettier --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Prettier updates" bash -c "npm view prettier version 2>/dev/null | head -1"
}
