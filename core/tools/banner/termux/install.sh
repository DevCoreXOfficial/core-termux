...
CORE_BANNER_MARKER="# ===== Core Banner ====="
...
EOF
	...
	log_success "Core Banner installed"
...
_uninstall_banner_impl() {
	...
	if [[ -n "$marker_line" ]]; then
		...
		log_success "Core Banner uninstalled"
	else
...
if [[ "${1:-}" == "update" ]]; then update_banner; fi
if [[ "${1:-}" == "reinstall" ]]; then reinstall_banner; fi
