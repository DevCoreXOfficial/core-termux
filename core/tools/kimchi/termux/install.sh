...
_compile_kimchi_helper_impl() {
  local HELPER_SRC="$CORE_TOOL_DIR/helper/kimchi_helper.c"
...
_kimchi_create_proot_wrapper() {
  local wrapper_src="$CORE_TOOL_DIR/bin/kimchi.proot"
...
_kimchi_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_TOOL_DIR/bin/kimchi"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_kimchi; fi
if [[ "${1:-}" == "uninstall" ]]; then uninstall_kimchi; fi
if [[ "${1:-}" == "update" ]]; then update_kimchi; fi
if [[ "${1:-}" == "reinstall" ]]; then reinstall_kimchi; fi
if [[ "${1:-}" == "version-local" ]]; then _get_installed_version kimchi; fi
if [[ "${1:-}" == "version-remote" ]]; then _get_remote_github_version getkimchi/kimchi; fi
