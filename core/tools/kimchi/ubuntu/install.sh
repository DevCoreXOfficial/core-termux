...
_gh_fetch() {
  ...
  re="kimchi_linux_(${arch}|${alt})"
  asset=$(curl -fsSL "https://api.github.com/repos/getkimchi/kimchi/releases/latest" \
    | jq -r --arg re "$re" '.assets[].name | select(test($re))' | head -1)
  [[ -z "$asset" ]] && return 1
  curl -fsSL "https://github.com/getkimchi/kimchi/releases/latest/download/${asset}" -o "/tmp/kimchi.dl" &>>"$LOG_FILE"
}
...
_impl_install() {
  ...
  tar -xzf /tmp/kimchi.dl -C /tmp && BIN=$(find /tmp -maxdepth 2 -type f -name kimchi | head -1) && [ -n "$BIN" ] && mv "$BIN" "$HOME/.local/bin/kimchi"
...
_impl_vlocal() {
  "$HOME/.local/bin/kimchi" --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1
}

_impl_vremote() {
  curl -fsSL https://api.github.com/repos/getkimchi/kimchi/releases/latest | grep '"tag_name"' | cut -d'"' -f4
}
