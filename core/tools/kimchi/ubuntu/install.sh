...
_impl_install() {
  ...
  loading "Installing Kimchi" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  command -v "kimchi" >/dev/null 2>&1 && rm -f "$(command -v kimchi)"
}
...
_impl_vlocal() {
  _get_installed_version "kimchi" "--version" "Kimchi"
}
...
case "${1:-}" in
  ...
  update)     _check_update_needed "Kimchi" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
