...
_impl_install() {
  ...
  loading "Installing Command Code" _impl_install_impl
}

_impl_install_impl() {
_impl_require_npm
  mkdir -p "$HOME/.local/bin"
  _npm_g install -g command-code &>>"$LOG_FILE"
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v command-code 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
    rm -f "$bin_path" || $CORE_SUDO rm -f "$bin_path" || true
  fi
  _npm_g uninstall -g command-code &>>"$LOG_FILE" || true
}
...
_impl_update_impl() {
  _npm_g install -g command-code@latest
}

_impl_vlocal() {
  _spin_capture "Detecting Command Code version" bash -c 'command -v command-code >/dev/null 2>&1 && command-code --version 2>/dev/null | grep -oE "[0-9]+\.[0-9]+[^ ]*" | head -1'
}

_impl_vremote() {
  _spin_capture "Checking Command Code updates" bash -c "npm view command-code version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Command Code" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
