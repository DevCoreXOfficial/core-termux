...
_impl_install() {
  separator
  box_large "Installing Live Server"
  separator
  echo

  loading "Installing Live Server" _impl_install_impl
}
...
_impl_update() {
  _impl_require_npm
  loading "Updating Live Server" _impl_update_impl || { log_error "Failed to update Live Server"; return 1; }
...
_impl_vremote() {
  _spin_capture "Checking Live Server updates" bash -c "npm view live-server version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "Live Server" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
