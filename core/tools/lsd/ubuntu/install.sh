...
_impl_install() {
  ...
  loading "Installing LSD" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Lsd version" bash -c "dpkg -s lsd 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Lsd updates" bash -c "apt-cache policy lsd 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
