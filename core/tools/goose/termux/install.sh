...
_compile_goose_helper_impl() {
  local HELPER_SRC="$CORE_TOOL_DIR/helper/goose_helper.c"
...
_goose_create_proot_wrapper() {
  local wrapper_src="$CORE_TOOL_DIR/bin/goose.proot"
...
_goose_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_TOOL_DIR/bin/goose"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_goose; fi
