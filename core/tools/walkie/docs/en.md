...
## Core's own agent (runs `core agent run -p <prompt> -y`)
...
```bash
core install walkie      # install
core update walkie       # update
core uninstall walkie    # remove
```
...
- Spanish docs (when available): `core show walkie:es`.
