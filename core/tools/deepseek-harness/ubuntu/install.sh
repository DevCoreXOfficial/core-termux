...
_impl_install() {
  ...
  loading "Installing DeepSeek Harness" _impl_install_impl
}
...
_impl_uninstall() {
  ...
  separator
  local bin_path
  bin_path="$(command -v deepseek-harness 2>/dev/null)"
  if [[ -n "$bin_path" ]]; then
...
_impl_update_impl() {
  _npm_g install -g @deepseek-ai/dsh@latest
}
...
_impl_vremote() {
  _spin_capture "Checking Deepseek Harness updates" bash -c "npm view @deepseek-ai/dsh version 2>/dev/null | head -1"
}

case "${1:-}" in
  install)    _impl_install ;;
  uninstall)  _impl_uninstall ;;
  update)     _check_update_needed "DeepSeek Harness" "$(_impl_vlocal)" "$(_impl_vremote)" _impl_update ;;
