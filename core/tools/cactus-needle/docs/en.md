...
- The install method is recorded in `~/.local/share/core-data/cactus-needle/.install-method`
- The JAX cache (`~/.cache/jax`) is shared with other JAX workloads and is not removed on uninstall
- Data directory: `~/.local/share/core-data/cactus-needle/`

## How to use it?

```bash
core install cactus-needle      # install
core update cactus-needle       # update
core uninstall cactus-needle    # remove
```
...
- Spanish docs (when available): `core show cactus-needle:es`.
