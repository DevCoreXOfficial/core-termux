...
_impl_install() {
  ...
  loading "Installing Python" _impl_install_impl
}
...
_impl_vlocal() {
  _spin_capture "Detecting Python version" bash -c "dpkg -s python3 2>/dev/null | grep '^Version:' | awk '{print $2}' | head -1"
}

_impl_vremote() {
  _spin_capture "Checking Python updates" bash -c "apt-cache policy python3 2>/dev/null | grep 'Candidate:' | awk '{print $2}' | head -1"
}
