...
_cursor_create_proot_wrapper() {
  local wrapper_src="$CORE_PATH/tools/ai/cursor/bin/cursor.proot"
...
_cursor_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$CORE_PATH/tools/ai/cursor/bin/cursor"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
if [[ "${1:-}" == "install" ]]; then install_cursor_cli; fi
