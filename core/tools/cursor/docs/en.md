...
- **Name:** Cursor CLI
- **Tags:** terminal, theme, color
- **Project:** https://cursor.com/cli
...
Cursor CLI (cursor-agent) is an AI-powered coding agent from Cursor that operates directly in your terminal. It provides intelligent code generation, editing, and debugging assistance. Since Cursor is not open source, the binary is downloaded directly from Cursor's CDN. The package is a Node.js application bundled with its own Node.js runtime. Core offers three installation methods: native with glibc support for best performance, native + proot to bypass "bad system call" errors, or via proot-distro Ubuntu container for maximum compatibility.
...
- The package is stored in `~/.local/share/core-data/cursor/`
...
```bash
core install cursor      # install
core update cursor       # update
core uninstall cursor    # remove
```
...
- Spanish docs (when available): `core show cursor:es`.
